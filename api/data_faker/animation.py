...
from api.src.services import IPService, UserService, AnimationService, StorageService
...
async def main():
    ...
    for _ in range(NEW_ROWS):
        data = {
            ...
            'image_ids': {
                'vertical': fake3l.random_element(elements=limbo_image_ids)
            },
            'produced_by': fake3l.company(),
            'released_at': fake3l.date_time(),
            'type': fake3l.random_element(elements=['TV', 'MOVIE', 'SP', 'OVA', 'OAD']),
            'episodes_num': fake3l.random_element(elements=[12, 24, 48, 52]),
            'sharing_addresses': {
                'baidu_cloud': {
                    'url': fake3l.url(),
        ...
        await animation_service.create(**data)
