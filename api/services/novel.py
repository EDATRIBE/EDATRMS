...
class NovelService:
    ...
    async def create(self, **data):
        async with self.db.acquire() as conn:
            result = await conn.execute(sasql.insert(NovelModel).values(**data))
            id = result.lastrowid
    ...
    async def edit(self, id, **data):
        ...
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.update(NovelModel).where(NovelModel.c.id == id).
                    values(**data)
            )
    ...
    async def delete(self, id):
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.delete(NovelModel).where(NovelModel.c.id == id))
    ...
    async def info(self, id):
        ...
        async with self.db.acquire() as conn:
            result = await conn.execute(
                NovelModel.select().where(NovelModel.c.id == id)
            )
    ...
    async def infos(self, ids):
        ...
        if valid_ids:
            async with self.db.acquire() as conn:
                result = await conn.execute(
                    NovelModel.select().where(NovelModel.c.id.in_(valid_ids))
                )
    ...
    async def list_novels(self, *,ip_id=None, limit=None, offset=None):
        select_sm = NovelModel.select()
        count_sm = sasql.select([sasql.func.count()]). \
            select_from(NovelModel)

        # select_sm = select_sm.order_by(UserModel.c.id.desc())
        if ip_id is not None:
            clause = NovelModel.c.ip_id == ip_id
