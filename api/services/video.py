...
class VideoService:
    ...
    async def create(self, **data):
        async with self.db.acquire() as conn:
            result = await conn.execute(sasql.insert(VideoModel).values(**data))
            id = result.lastrowid
    ...
    async def edit(self, id, **data):
        ...
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.update(VideoModel).where(VideoModel.c.id == id).
                    values(**data)
            )
    ...
    async def delete(self, id):
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.delete(VideoModel).where(VideoModel.c.id == id))
    ...
    async def info(self, id):
        ...
        async with self.db.acquire() as conn:
            result = await conn.execute(
                VideoModel.select().where(VideoModel.c.id == id)
            )
    ...
    async def infos(self, ids):
        ...
        if valid_ids:
            async with self.db.acquire() as conn:
                result = await conn.execute(
                    VideoModel.select().where(VideoModel.c.id.in_(valid_ids))
                )
    ...
    async def list_animations(self, *,animation_id=None, limit=None, offset=None):
        ...
        if animation_id is not None:
            clause = VideoModel.c.animation_id == animation_id
