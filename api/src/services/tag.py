...
class TagService:
    ...
    async def create(self, **data):
        async with self.db.acquire() as conn:
            result = await conn.execute(sasql.insert(TagModel).values(**data))
            id = result.lastrowid
    ...
    async def edit(self, id, **data):
        ...
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.update(TagModel).where(TagModel.c.id == id).
                    values(**data)
            )
    ...
    async def delete(self, id):
        async with self.db.acquire() as conn:
            await conn.execute(
                sasql.delete(TagModel).where(TagModel.c.id == id))
    ...
    async def info(self, id):
        ...
        async with self.db.acquire() as conn:
            result = await conn.execute(
                TagModel.select().where(TagModel.c.id == id)
            )
    ...
    async def infos(self, ids):
        ...
        if valid_ids:
            async with self.db.acquire() as conn:
                result = await conn.execute(
                    TagModel.select().where(TagModel.c.id.in_(valid_ids))
                )
    ...
    async def list_tags(self, *, limit=None, offset=None):
        select_sm = TagModel.select()
