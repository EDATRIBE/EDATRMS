...
async def upload(request):
    ...
    if region == StorageRegion.LOCAL.value:
        ...
        return response_json(
            code=ResponseCode.FAILURE, message="This region is not available"
        )

    return response_json(files=[FileSchema().dump(v) for v in saved_files])
...
async def filepond_upload(request):
    ...
    if uploaded_file is None:
        return response_json(
            code=ResponseCode.DIRTY, message="Missing field: file_pond_file"
        )
...
async def filepond_load(request, id):
    ...
    return await response.file(
        file_posi, filename=file.get("file_meta", {}).get("name", "")
    )
