...
async def create(request):
    ...
    required_field_validation(
        data=data,
        required_field=["ip_id", "name", "released_at", "type", "episodes_num"],
    )
    ...
    for key, value in data.get("image_ids", {}).items():
        ...
        if file is None:
            return response_json(
                code=ResponseCode.DIRTY, message="Missing file: " + key
            )
    ...
    animation = await animation_service.edit(animation["id"], image_ids=new_image_ids)
...
async def list_all(request):
    ...
    return response_json(
        animations=await dump_animation_infos(request, animations), total=total
    )
