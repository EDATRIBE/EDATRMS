...
async def list_announcements(request):
    ...
    for md_file_path in md_file_path_list:
        ...
        title_and_mark = name.split(
            request.app.config['ANNOUNCEMENTS_ORDER_PREFIX'], 1
        )
        ...
        announcement.uri = md_file_path.replace(
            os.path.join(request.app.config['DATA_PATH'],request.app.config['ANNOUNCEMENTS_DIR']),
            'announcements'
        )
    ...
    sorted_announcements_list = sorted_marked_announcements_list + sorted_unmarked_announcements_list
