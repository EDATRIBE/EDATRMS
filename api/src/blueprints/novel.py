...
async def create(request):
    ...
    required_field_validation(
        data=data, required_field=["ip_id", "name", "volumes_num", "integrated"]
    )
    ...
    novel = await novel_service.edit(novel["id"], image_ids=new_image_ids)
...
async def list_all(request):
    ...
    return response_json(novels=await dump_novel_infos(request, novels), total=total)
