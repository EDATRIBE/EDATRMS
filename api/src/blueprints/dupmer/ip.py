...
async def dump_ip_info(request, ip):
    ...
    animation_vertical_image_ids = [
        animation["image_ids"].get("vertical") for animation in animations
    ]
    animation_horizontal_image_ids = [
        animation["image_ids"].get("horizontal") for animation in animations
    ]
    animation_vertical_images = await storage_service.infos(
        animation_vertical_image_ids
    )
    animation_horizontal_images = await storage_service.infos(
        animation_horizontal_image_ids
    )
    for (
        animation,
        videos,
        captions,
        animation_vertical_image,
        animation_horizontal_image,
    ) in zip(
        animations,
        videos_list,
        captions_list,
    ...
    novel_horizontal_image_ids = [
        novel["image_ids"].get("horizontal") for novel in novels
    ]
    ...
    for novel, novel_vertical_image, novel_horizontal_image in zip(
        novels, novel_vertical_images, novel_horizontal_images
    ):
        novel["images"] = {
...
async def dump_ip_infos(request, ips):
    ...
    for ip, animations, novels, tag_ids in zip(
        ips, animations_list, novels_list, tag_ids_list
    ):
        ip["tag_ids"] = tag_ids
        ip["animations"] = animations
        ip["novels"] = novels
    ...
    captions_list = await caption_service.infos_list_by_animations_ids(
        all_animation_ids
    )
    all_animation_vertical_image_ids = [
        animation["image_ids"].get("vertical") for animation in all_animations
    ]
    all_animation_horizontal_image_ids = [
        animation["image_ids"].get("horizontal") for animation in all_animations
    ]
    all_animation_vertical_images = await storage_service.infos(
        all_animation_vertical_image_ids
    )
    all_animation_horizontal_images = await storage_service.infos(
        all_animation_horizontal_image_ids
    )
    for (
        animation,
    ...
    all_novel_vertical_image_ids = [
        novel["image_ids"].get("vertical") for novel in all_novels
    ]
    all_novel_horizontal_image_ids = [
        novel["image_ids"].get("horizontal") for novel in all_novels
    ]
    all_novel_vertical_images = await storage_service.infos(
        all_novel_vertical_image_ids
    )
    all_novel_horizontal_images = await storage_service.infos(
        all_novel_horizontal_image_ids
    )
    for novel, novel_vertical_image, novel_horizontal_image in zip(
        all_novels, all_novel_vertical_images, all_novel_horizontal_images
    ):
        novel["images"] = {
            "vertical": novel_vertical_image,
            "horizontal": novel_horizontal_image,
