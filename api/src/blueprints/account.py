...
async def login(request):
    ...
    if user is None or sha256_hash(data["password"], user["salt"]) != user["password"]:
        return response_json(
            code=ResponseCode.FAILURE, message="Incorrect name or password"
        )
...
async def edit(request):
    ...
    data = sift_dict_by_key(
        data=data, allowed_key=["password", "qq", "email", "intro", "avatar_id"]
    )

    cond1 = data.get("avatar_id") is not None
    cond2 = request.ctx.session["user"].get("avatar") is None or request.ctx.session[
        "user"
    ].get("avatar").get("id") != data.get("avatar_id")
    if cond1 and cond2:
        ...
        if file is None:
            return response_json(
                ...
                message="Missing file: " + data.get("avatar_id"),
            )
    ...
    user_service = UserService(request.app.config, request.app.db, request.app.cache)
    user = await user_service.edit(request.ctx.session["user"]["id"], **data)
