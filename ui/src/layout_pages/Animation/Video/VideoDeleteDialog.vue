            ...
            <div style="width: 100%" class="q-pl-md bl">
                ...
                <div class="row q-pb-md">
                    <p class="q-my-none text-body1 text-white ">
                        Please input
                        <span class="text-red text-weight-bold">{{ this.videoName}}</span>
            ...
            <div style="width: 100%" class="q-pl-md">
                <div class="row justify-end">
                    ...
                    <q-btn
                        ...
                        :disable="!isValid"
                        @click="commitVideoDelete(video.id)"
                    >
                        commit
                    </q-btn>
                </div>
...
export default {
    ...
    methods: {
        commitVideoDelete(id) {
            let data = {id: id}
            this.$axios.post('api/video/delete', data).then((response) => {
    ...
    computed: {
        isValid(){
            return this.confirmation === this.videoName
        },
        initialized() {
        ...
        videoName() {
            if(this.video===null){
                return null
            }else if (this.video.fileMeta.name===''){
                return 'video'
            }else {
                return this.video.fileMeta.name
            }
        }
    }
}
